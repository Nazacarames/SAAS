...
export const recordInboundPayloadReplayKeyReuseByIpDetected = (context?: Record<string, unknown>) => {
  bumpHardeningMetric("inbound.payload_replay_key_reuse_by_ip_detected");
  pushHardeningSignal("inbound_payload_replay_key_reuse_by_ip_detected", 2, context);
};
...
export const recordInboundPayloadReplayGuardMemoryFallbackUsed = (context?: Record<string, unknown>) => {
  bumpHardeningMetric("inbound.payload_replay_guard_memory_fallback_used");
  pushHardeningSignal("inbound_payload_replay_guard_memory_fallback_used", 2, context);
};
...
const sendManagedReply = async ({ ticket, contact, text }: { ticket: any; contact: any; text: string }) => {
  ...
  // Provider idempotency key should be stable only within this logical send attempt (and its retries),
  // not across future messages that happen to have the same semantic dedupe key.
  const providerIdempotencyKey = `mr:${crypto.randomUUID()}`;
  bumpHardeningMetric("outbound.managed_reply_provider_idempotency_key_issued");
  const sendResult = await sendCloudText(String(contact.number), cleanText, providerIdempotencyKey);
