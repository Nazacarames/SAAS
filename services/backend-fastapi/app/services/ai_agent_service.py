...
async def search_properties(location: str = None, price_max: int = None,
                     property_type: str = None, rooms: int = None,
                     operation: str = None) -> List[Dict]:
    """
    Buscar propiedades en Tokko API con FILTRADO REAL.

    Bug histórico (reporte 18/7, caso DUNOD): esta función traía las primeras
    20 propiedades de la cartera sin ningún filtro ("let AI filter") — si en
    esas 20 no había locales/oficinas en alquiler ni la zona pedida, el bot
    respondía que no había nada. Ahora se pagina más ancho y se filtra acá por
    operación, tipo, zona, precio y ambientes; la IA recibe solo lo que matchea.
    """
    try:
        tokko_url = settings.tokko_api_url or "https://api.tokkobroker.com/api/v1"
        params = {
            "key": settings.tokko_api_key,
            "limit": 200,  # página ancha; el filtrado de abajo reduce a 20
        }

        async with httpx.AsyncClient() as client:
            response = await client.get(f"{tokko_url}/property/", params=params, timeout=25)
        if response.status_code == 200:
            ...
            TYPE_MAP = {
                ...
                "house": ["casa", "ph", "dúplex", "duplex"],
                "store": ["local", "fondo de comercio"],
                "office": ["oficina"],
                "land": ["terreno", "lote", "campo"],
            }

            def _matches(p):
                # operación: venta / alquiler / alquiler temporario
                if operation:
                    want = _norm(operation)
                    ops = [_norm(str(op.get("operation_type", ""))) for op in (p.get("operations") or [])]
                    if want.startswith("alqu") and not any("alquiler" in o or "rent" in o for o in ops):
                        return False
                    if want.startswith("vent") and not any("venta" in o or "sale" in o for o in ops):
                        return False
                # tipo de propiedad
                if property_type:
                    tipo = p.get("type")
                    tipo_name = _norm(tipo.get("name") if isinstance(tipo, dict) else str(tipo))
                    aliases = TYPE_MAP.get(_norm(property_type), [_norm(property_type)])
                    if not any(a in tipo_name for a in aliases):
                        return False
                # zona (substring sobre la localización completa)
                if location:
                    full = _norm((p.get("location") or {}).get("full_location", "")) + " " + _norm(p.get("address", ""))
                    if _norm(location) not in full:
                        return False
                # precio máximo (sobre cualquier operación publicada)
                if price_max:
                    precios = [pi.get("price", 0) for op in (p.get("operations") or []) for pi in (op.get("prices") or [])]
                    if precios and min(pr for pr in precios if pr) > price_max:
                        return False
                # ambientes/dormitorios
                if rooms:
                    amb = p.get("room_amount") or p.get("bedroom_amount") or 0
                    if amb and int(amb) < int(rooms):
                        return False
                return True

            properties = [p for p in data.get("objects", []) if _matches(p)][:20]
            if properties:
                results = []
                for p in properties:
                    # Get photo
        ...
        "function": {
            ...
            "parameters": {
                ...
                "properties": {
                    ...
                    "operation": {
                        "type": "string",
                        "description": "Operación buscada: 'venta' o 'alquiler'. SIEMPRE indicarla si el usuario la menciona (ej: 'local en alquiler' → operation='alquiler', property_type='store')"
                    }
                }
            }
        }
...
def get_openai_client():
    """Cliente LLM (OpenAI o MiniMax según config). Centralizado en services/llm."""
    from app.services import llm
...
"""
        ...
        response = openai_client.chat.completions.create(
            model=llm.model_for("gpt-4o-mini"),
            messages=messages,
            tools=FUNCTIONS,
            tool_choice="auto",
